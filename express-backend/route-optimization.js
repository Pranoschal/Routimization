...
app.post("/optimize-tours", async (req, res) => {
  const { deliveries, vehicleStartLocation, globalStartTime, globalEndTime } = req.body;
 
  if (!deliveries || !vehicleStartLocation || !globalStartTime || !globalEndTime) {
  ...
  try {
    ...
    for (let i = 0 ; i<response[0].routes[0].visits.length; i++) {
      if(response[0].routes[0].visits[i].visitLabel){
        ...
        locationName.push({storename : storename, campaignName : campaignName, staus : true})
    ...
    Myresponse.totalTravelTime = secondsToHours(Number(response[0].metrics.aggregatedRouteMetrics.totalDuration.seconds))
